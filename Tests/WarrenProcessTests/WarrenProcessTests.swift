...
final class WarrenProcessTests: XCTestCase {
    func testSingleInstanceLockRejectsConcurrentOwnersAndCanBeReacquired() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("warren-instance-lock-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: directory, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: directory) }
        let lockURL = directory.appendingPathComponent("application.lock")

        var owner: WarrenSingleInstanceLock? = try XCTUnwrap(
            WarrenSingleInstanceLock(fileURL: lockURL)
        )
        XCTAssertNotNil(owner)
        for _ in 0..<5 {
            XCTAssertNil(WarrenSingleInstanceLock(fileURL: lockURL))
        }
        owner = nil
        XCTAssertNotNil(WarrenSingleInstanceLock(fileURL: lockURL))
    }

    func testHeadlessProductStartsEmptyAndExitsWithoutCreatingSessions() async throws {
        guard let executable = ProcessInfo.processInfo.environment["WARREN_APP_EXECUTABLE"] else {
            throw XCTSkip("WARREN_APP_EXECUTABLE is provided by scripts/verify.sh")
        }
    ...
    func testHeadlessProductRejectsSecondInstanceAndQuitKeepsTmuxAlive() async throws {
        ...
        let first = try fixture.launch(
            ...
            extraEnvironment: [
                "WARREN_HEADLESS_HOLD_MILLISECONDS": "500",
                "WARREN_HEADLESS_CREATE_SESSION_PATH": fixture.workspaceURL.path,
            ]
        )
...
private final class ProcessFixture: @unchecked Sendable {
    ...
    init() throws {
        root = FileManager.default.temporaryDirectory
            .appendingPathComponent("warren-process-\(UUID().uuidString)", isDirectory: true)
        ...
        tmuxSocketName = "warren-test-\(UUID().uuidString.lowercased())"
    ...
    func launch(
        ...
        process.environment = ProcessInfo.processInfo.environment.merging([
            "WARREN_HEADLESS_ACCEPTANCE": "1",
            "WARREN_STATE_DATABASE": databaseURL.path,
            "WARREN_RUNTIME_OUTPUT_DIRECTORY": runtimeURL.path,
            "WARREN_INSTANCE_LOCK": lockURL.path,
            "WARREN_HEADLESS_REPORT": reportURL.path,
            "WARREN_TMUX_SOCKET_NAME": tmuxSocketName,
    ...
    func tmuxSessionExists(sessionID: String) -> Bool {
        runTmux(["has-session", "-t", "warren-\(sessionID)"]) == 0
    }
    ...
    func killTmuxSession(sessionID: String) {
        ...
        _ = runTmux(["kill-session", "-t", "warren-\(sessionID)"])
    }
