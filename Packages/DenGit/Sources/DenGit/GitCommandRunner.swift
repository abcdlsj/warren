...
/// Low-level process runner for git commands.
/// Binary resolution is cached because it is stable for the life of the app.
public actor GitCommandRunner {
    ...
    public func run(in directory: String, _ arguments: [String]) async throws -> String {
        // Prefer `git -C` so we do not need to mutate process cwd globally.
