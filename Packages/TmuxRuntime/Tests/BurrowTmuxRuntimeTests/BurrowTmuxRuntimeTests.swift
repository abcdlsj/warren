...
import BurrowDomain
import BurrowHost
@testable import BurrowTmuxRuntime

final class BurrowTmuxRuntimeTests: XCTestCase {
    ...
    func testSessionNamingIsStableAndRoundTrips() {
        ...
        XCTAssertEqual(name, "burrow-aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa")
        XCTAssertTrue(TmuxSessionNaming.isBurrowName(name))
        XCTAssertEqual(TmuxSessionNaming.sessionID(from: name), sessionID)
        XCTAssertFalse(TmuxSessionNaming.isBurrowName("burrow-main"))
    }

    func testHostedTerminalEnvironmentDropsLauncherPolicyAndIdentity() {
        let environment = BurrowTerminalEnvironment.sanitized(from: [
            "PATH": "/usr/bin:/bin",
            "HOME": "/Users/example",
            "TERM": "xterm-ghostty",
        ...
        XCTAssertEqual(environment["TERM_PROGRAM"], "Burrow")
    ...
    func testCreateDeclaresHostedTerminalIdentityWithoutOverridingTerm() async throws {
        ...
        XCTAssertTrue(arguments.contains("TERM_PROGRAM=Burrow"))
        ...
        XCTAssertTrue(shellCommand.contains("'TERM_PROGRAM=Burrow'"))
    ...
    func testCreateAndWriteUsesBinarySafeTmuxBuffer() async throws {
        ...
        XCTAssertTrue(loadBuffer.hasPrefix("burrow-input-\(sessionID.description)-"))
    ...
    func testLocalTmuxSmokeWhenAvailable() async throws {
        ...
        XCTAssertEqual(values["TERM_PROGRAM"], "Burrow")
        ...
        let environmentOutput = try await waitForSpool(
            containing: Data("TERM_PROGRAM=Burrow".utf8),
            at: outputURL
        )
        ...
        let marker = Data("DEN_SMOKE_\(UUID().uuidString)".utf8)
    ...
    func testLocalTmuxAdapterRestartAdoptsSpoolTail() async throws {
        ...
        let markerBefore = Data("DEN_RESTART_BEFORE_\(UUID().uuidString)".utf8)
        ...
        let markerAfter = Data("DEN_RESTART_AFTER_\(UUID().uuidString)".utf8)
    ...
    private func temporaryDirectory() throws -> URL {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("burrow-tmux-runtime-\(UUID().uuidString)", isDirectory: true)
