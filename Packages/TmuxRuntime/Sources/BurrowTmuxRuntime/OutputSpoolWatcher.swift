    ...
    init(
        ...
        do {
            ...
            throw TmuxRuntimeError.outputSpoolUnavailable(
                ...
                recovery: "Ensure Burrow can read the output directory, then delete it and retry the session."
            )
        }

        self.onBytes = onBytes
        let fd = fileHandle.fileDescriptor
        let queue = DispatchQueue(label: "dev.burrow.tmux-output.\(UUID().uuidString)")
        self.queue = queue
