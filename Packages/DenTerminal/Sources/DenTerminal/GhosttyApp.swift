...
/// Actions that Den intercepts from ghostty keybindings.
/// Ghostty maps keys to these intents; Den provides the tmux implementation.
...
final class GhosttyApp {
    ...
    /// Callback for ghostty actions that Den intercepts (tabs, splits, etc.).
    ...
    /// Build a ghostty config: load user's config first, then apply Den overrides.
    ...
    private nonisolated static func onAction(
        ...
        let denAction: GhosttyAppAction
        switch action.tag {
        ...
        case GHOSTTY_ACTION_GOTO_TAB:
            ...
            switch raw {
            case GHOSTTY_GOTO_TAB_PREVIOUS: denAction = .gotoTab(.previous)
            case GHOSTTY_GOTO_TAB_NEXT: denAction = .gotoTab(.next)
            case GHOSTTY_GOTO_TAB_LAST: denAction = .gotoTab(.last)
            default:
                let index = Int(raw.rawValue)
                guard index > 0 else { return false }
                denAction = .gotoTab(.index(index))
            }
        case GHOSTTY_ACTION_NEW_SPLIT:
            switch action.action.new_split {
            case GHOSTTY_SPLIT_DIRECTION_RIGHT: denAction = .newSplit(.right)
            case GHOSTTY_SPLIT_DIRECTION_DOWN: denAction = .newSplit(.down)
            case GHOSTTY_SPLIT_DIRECTION_LEFT: denAction = .newSplit(.left)
            case GHOSTTY_SPLIT_DIRECTION_UP: denAction = .newSplit(.up)
            default: return false
            }
        case GHOSTTY_ACTION_GOTO_SPLIT:
            switch action.action.goto_split {
            case GHOSTTY_GOTO_SPLIT_PREVIOUS: denAction = .gotoSplit(.previous)
            case GHOSTTY_GOTO_SPLIT_NEXT: denAction = .gotoSplit(.next)
            case GHOSTTY_GOTO_SPLIT_UP: denAction = .gotoSplit(.up)
            case GHOSTTY_GOTO_SPLIT_DOWN: denAction = .gotoSplit(.down)
            case GHOSTTY_GOTO_SPLIT_LEFT: denAction = .gotoSplit(.left)
            case GHOSTTY_GOTO_SPLIT_RIGHT: denAction = .gotoSplit(.right)
            default: return false
            }
        case GHOSTTY_ACTION_RESIZE_SPLIT:
            ...
            denAction = .resizeSplit(dir, amount: rs.amount)
        case GHOSTTY_ACTION_EQUALIZE_SPLITS:
            denAction = .equalizeSplits
        case GHOSTTY_ACTION_TOGGLE_SPLIT_ZOOM:
            denAction = .toggleSplitZoom
        case GHOSTTY_ACTION_NEW_WINDOW:
            denAction = .newWindow
        case GHOSTTY_ACTION_CLOSE_WINDOW:
            denAction = .closeWindow
        case GHOSTTY_ACTION_OPEN_CONFIG:
            denAction = .openConfig
        case GHOSTTY_ACTION_TOGGLE_FULLSCREEN:
            denAction = .toggleFullscreen
        default:
            return false
        }
        ...
        DispatchQueue.main.async {
            MainActor.assumeIsolated {
                GhosttyApp.shared.actionHandler?(denAction)
            }
        }
        return true
    }
