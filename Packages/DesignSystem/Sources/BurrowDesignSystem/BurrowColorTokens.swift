...
///
/// Values are copied from Superset's default Ember theme. Burrow intentionally
/// has no light appearance branch in the current product scope.
public struct BurrowColorTokens: Sendable {
    ...
    /// `bg-muted/45 dark:bg-muted/35`; Burrow is dark-only, so keep the dark
    ...
    /// Burrow currently ships only Superset's default Ember dark appearance.
...
public enum BurrowWashKind: Sendable {
    case hover
    case selected
    case tertiary
}

public extension BurrowColorTokens {
    func wash(_ kind: BurrowWashKind) -> Color {
        switch kind {
        case .hover:
            fillHover
        case .selected:
