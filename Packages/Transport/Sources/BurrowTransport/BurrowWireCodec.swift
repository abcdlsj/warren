...
public enum BurrowDecodedBinaryMessage: Hashable, Sendable {
    case input(BurrowDecodedInputFrame)
    case output(BurrowDecodedOutputFrame)
...
public struct BurrowDecodedInputFrame: Hashable, Sendable {
...
public struct BurrowDecodedOutputFrame: Hashable, Sendable {
...
public struct BurrowWireCodec: Sendable {
    ...
    public init(
        maxControl: Int = BurrowWireCodec.defaultMaxControl,
        maxHeader: Int = BurrowWireCodec.defaultMaxHeader,
        maxPayload: Int = BurrowWireCodec.defaultMaxPayload
    ) {
    ...
    public func decodeFrame(_ bytes: [UInt8]) throws -> BurrowDecodedBinaryMessage {
    ...
    private func encodeControlValue<Value: Encodable>(_ value: Value) throws -> [UInt8] {
        ...
        do {
            ...
            throw BurrowWireCodecError.invalidControlJSON
        }
        guard data.count <= maxControl else {
            throw BurrowWireCodecError.controlTooLarge(actual: data.count, limit: maxControl)
        }
        return Array(data)
    }
    ...
    private func decodeControlValue<Value: Decodable>(
        ...
        guard bytes.count <= maxControl else {
            throw BurrowWireCodecError.controlTooLarge(actual: bytes.count, limit: maxControl)
        }
