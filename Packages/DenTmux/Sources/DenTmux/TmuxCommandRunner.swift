...
/// Low-level process runner for tmux commands.
/// It loads a login-shell environment once so Homebrew/MacPorts installs can be discovered.
    ...
    private func loadShellEnvironment() async -> [String: String] {
        ...
        do {
            ...
            if exitCode == 0 {
                ...
                // Reconstruct the shell environment from `env` output rather than relying on the parent process PATH.
    ...
    public func run(_ arguments: [String]) async throws -> String {
        ...
        if exitCode != 0 {
            ...
            if exitCode == 1 && stdout.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                // Some tmux commands return exit 1 with empty output when there is simply nothing to list.
                return ""
            }
