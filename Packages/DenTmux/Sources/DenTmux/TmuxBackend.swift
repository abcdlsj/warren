...
/// Concrete tmux backend built on top of `TmuxCommandRunner`.
public actor TmuxBackend: TmuxControlling {
    ...
    public func pollOnce() async {
        do {
            ...
            // Polling is best-effort; a future tick will retry.
    ...
    public func listSessionNames() async -> [String] {
        let output = (try? await runner.run("list-sessions", "-F", "#{session_name}")) ?? ""
        return output
    ...
    public func scanAll() async throws -> [TmuxSession] {
        ...
        // tmux does not return nested windows/panes in one command, so enrich the session snapshot step by step.
    ...
    public func cloneSessionGroup(sourceSessionId: String, cloneSessionId: String) async throws -> TmuxSession {
        let output = try await runner.run(
            "new-session", "-d", "-t", sourceSessionId, "-s", cloneSessionId,
            "-P", "-F", TmuxParser.sessionFormat
        )
        ...
        guard let session = sessions.first else {
            throw TmuxError.executionFailed(
                command: "new-session",
                exitCode: -1,
                stderr: "Failed to parse cloned session"
            )
        }
        return session
    }
    ...
    public func splitPane(sessionId: String, paneId: String, horizontal: Bool, cwd: String?) async throws -> TmuxPane {
        // Passing an empty paneId means "split the currently active pane in this session".
