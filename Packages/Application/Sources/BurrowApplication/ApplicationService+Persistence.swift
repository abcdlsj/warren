...
private enum BurrowApplicationPerformance {
...
extension BurrowApplicationService {
    ...
    internal func save(_ candidate: PersistedHostState) async throws {
        do {
            ...
            throw BurrowApplicationError.repository(String(describing: error))
        }
    }
    ...
    private func scheduleSequencePersistence() {
        ...
        sequencePersistenceTask = Task { [weak self] in
            do {
                try await Task.sleep(nanoseconds: BurrowApplicationPerformance.sequencePersistenceDelay)
            } catch {
                return
            }
    ...
    internal func scheduleOutputPublication(for sessionID: TerminalSessionID) {
        ...
        outputPublishTask = Task { [weak self] in
            do {
                try await Task.sleep(nanoseconds: BurrowApplicationPerformance.outputPublicationDelay)
            } catch {
                return
            }
    ...
    internal func makeSnapshot() async -> BurrowApplicationSnapshot {
        ...
        var sessions: [BurrowApplicationSession] = []
        ...
        for connection in sorted {
            ...
            if let cached = outputSnapshotCache[connection.session.id],
               !invalidatedOutputSessions.contains(connection.session.id) {
                ...
                let refreshed = BurrowApplicationOutputSnapshot(
        ...
        for persisted in state.terminalSessions where !connectedSessionIDs.contains(persisted.id) {
            sessions.append(BurrowApplicationSession(
                id: persisted.id,
