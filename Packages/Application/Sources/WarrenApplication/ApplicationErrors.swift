...
public enum WarrenApplicationError: Error, Hashable, Sendable, LocalizedError {
    case alreadyStarted
    case notStarted
    case invalidLifecycle(WarrenApplicationLifecycle)
    ...
    public var errorDescription: String? {
        switch self {
        case .alreadyStarted:
            return "Warren is already running."
        case .notStarted:
            return "Warren is not running."
        case let .invalidLifecycle(lifecycle):
            return "Warren is in state \(lifecycle) and cannot perform this action."
    ...
    public var recoverySuggestion: String? {
        ...
        case .notStarted, .invalidLifecycle:
            return "Start Warren before managing projects or terminals."
        ...
        case .repository:
            return "Check read/write access to Warren's state directory, then retry."
        case .runtime:
...
public enum WarrenApplicationIssueSeverity: String, Codable, Hashable, Sendable {
    case info
    case warning
    case error
}
...
public struct WarrenApplicationIssue: Identifiable, Codable, Hashable, Sendable {
    public let id: String
    public let severity: WarrenApplicationIssueSeverity
    ...
    public init(
        id: String,
        severity: WarrenApplicationIssueSeverity = .warning,
        title: String,
        detail: String,
    ...
    public init(id: String, error: Error, recoverySuggestion: String = "Please try again.") {
        ...
        self.init(
            id: id,
            severity: .error,
            title: localized?.errorDescription ?? "Warren operation failed",
...
extension WarrenApplicationError {
    internal var issue: WarrenApplicationIssue {
        WarrenApplicationIssue(
            id: "application.\(String(describing: self))",
            severity: .error,
            title: errorDescription ?? "Warren operation failed",
...
extension Error {
    internal var asApplicationError: WarrenApplicationError {
        if let error = self as? WarrenApplicationError { return error }
