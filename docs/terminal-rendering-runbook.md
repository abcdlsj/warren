...
rg '"event":"(workspace_switch|terminal_tab_switch|terminal_view_appear|select_session|attach_start|attach_size|attach_complete|feed_output|present_now|present_stall_suspected|present_complete|present_wait_timeout|roster_apply|resize_request|viewport_sync)"' \
...
- `present_wait_timeout` shows a ready output sequence but an unready view:
  this is a desktop lifecycle/presentation failure. If both the rendered and
  target sequences are behind, continue with the Host/spool checks.
