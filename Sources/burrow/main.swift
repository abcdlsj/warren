...
// `burrow` CLI: talks to the running app's WebSocket relay so scripts and
// agents can create sessions, send input, and read replies headlessly.

let arguments = Array(CommandLine.arguments.dropFirst())

struct BurrowCLIError: Error, CustomStringConvertible {
...
func readToken() -> String {
    let defaults = UserDefaults(suiteName: "com.abcdlsj.burrow")
    if let token = defaults?.string(forKey: "webRelay.token"), !token.isEmpty {
        return token
    }
    fail("Missing webRelay.token; start Burrow.app first.")
}
...
final class RelayClient {
    ...
    init() throws {
        ...
        guard fd >= 0 else { throw BurrowCLIError(description: "socket failed") }
        ...
        guard result == 0 else { throw BurrowCLIError(description: "connect failed; is Burrow.app running?") }
        ...
        try? Data(request.utf8).write(to: URL(fileURLWithPath: "/tmp/burrow-cli-request.bin"))
        ...
        guard buffer.range(of: Data("\r\n\r\n".utf8)) != nil,
            ...
            throw BurrowCLIError(description: "bad websocket handshake")
        }
        ...
        guard header.contains("101 Switching Protocols") else {
            throw BurrowCLIError(description: header)
        }
    ...
    private func sendFrame(opcode: UInt8, payload: Data) throws {
        ...
        while offset < data.count {
            ...
            if written <= 0 { throw BurrowCLIError(description: "write failed") }
            offset += written
        }
    }
    ...
    private func readAvailable(timeout: TimeInterval) throws {
        ...
        guard ready > 0 else { throw BurrowCLIError(description: "timeout") }
        var chunk = [UInt8](repeating: 0, count: 64 * 1024)
        let count = read(fd, &chunk, chunk.count)
        guard count > 0 else { throw BurrowCLIError(description: "connection closed") }
...
func authRoster() throws -> (RelayClient, [String: Any]) {
    ...
    guard let object = try? JSONSerialization.jsonObject(with: payload) as? [String: Any],
        ...
        throw BurrowCLIError(description: "auth failed")
    }
...
func run() throws {
    guard let command = arguments.first else {
        print("usage: burrow session list | agent create session <title> | session send <id> <text> | session read <id> [--timeout N] [--contains TEXT]")
        return
    }
    ...
    if command == "agent", arguments.dropFirst().first == "create", arguments.dropFirst().dropFirst().first == "session" {
        ...
        if let workspaceID {
            ...
            throw BurrowCLIError(description: "no workspace available")
        }
    ...
    if command == "session", arguments.dropFirst().first == "send" {
        ...
        guard parts.count >= 2 else { throw BurrowCLIError(description: "usage: burrow session send <id> <text>") }
    ...
    if command == "session", arguments.dropFirst().first == "read" {
        let parts = Array(arguments.dropFirst(2))
        guard let sessionID = parts.first else { throw BurrowCLIError(description: "usage: burrow session read <id>") }
        ...
        print(output)
        if let needle, !output.contains(needle) {
            throw BurrowCLIError(description: "expected text not found: \(needle)")
        }
        return
    }

    throw BurrowCLIError(description: "unknown command: \(command)")
}
