...
enum BurrowNextWindowConfiguration {
    ...
    static func apply(to window: NSWindow) {
        ...
        // the 40pt chrome run edge-to-edge; Burrow draws its own traffic lights
...
struct BurrowNextWindowConfigurator: NSViewRepresentable {
    func makeNSView(context: Context) -> BurrowNextWindowConfigurationView {
        BurrowNextWindowConfigurationView()
    }

    func updateNSView(_ nsView: BurrowNextWindowConfigurationView, context: Context) {
        nsView.applyIfNeeded()
    }
}

@MainActor
final class BurrowNextWindowConfigurationView: NSView {
    ...
    func applyIfNeeded() {
        ...
        BurrowNextWindowConfiguration.apply(to: window)
    }
}
