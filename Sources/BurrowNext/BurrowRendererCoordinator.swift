...
protocol BurrowRendererService: Sendable {
    func sendInput(
...
extension BurrowApplicationService: BurrowRendererService {}

struct BurrowRendererSurfaceKey: Hashable, Sendable {
...
@MainActor
@Observable
final class BurrowRendererCoordinator {
    private(set) var mountedSurfaces: [GhosttySurface] = []

    @ObservationIgnored private let service: any BurrowRendererService
    @ObservationIgnored private let windowID: ClientWindowID
    @ObservationIgnored private var surfaces: [BurrowRendererSurfaceKey: GhosttySurface] = [:]
    ...
    @ObservationIgnored private var snapshot = BurrowApplicationSnapshot.empty()
    ...
    init(
        service: any BurrowRendererService,
        windowID: ClientWindowID = BurrowApplicationDefaults.mainWindowID
    ) {
    ...
    func reconcile(
        snapshot: BurrowApplicationSnapshot,
        ...
        let validKeys = Set(snapshot.sessions.compactMap { session -> BurrowRendererSurfaceKey? in
            ...
            return BurrowRendererSurfaceKey(
                windowID: windowID,
        ...
        for session in snapshot.sessions where visibleSessionIDs.contains(session.id) {
            ...
            let key = BurrowRendererSurfaceKey(
                windowID: windowID,
                workspaceID: session.workspaceID,
                sessionID: session.id
            )
        ...
        mountedSurfaces = visibleTabs.compactMap { tab in
            ...
            return surfaces[BurrowRendererSurfaceKey(
                windowID: windowID,
    ...
    func receiveInput(_ data: Data, from key: BurrowRendererSurfaceKey) async {
    ...
    func receiveResize(
        columns: Int,
        rows: Int,
        from key: BurrowRendererSurfaceKey
    ) async {
    ...
    private func sendInput(_ data: Data, key: BurrowRendererSurfaceKey) async {
        ...
        do {
            ...
            NSLog(
                "Burrow terminal input failed for %@: %@",
    ...
    private func drainResizes(for key: BurrowRendererSurfaceKey) async {
    ...
    private func disposeSurface(_ key: BurrowRendererSurfaceKey) {
