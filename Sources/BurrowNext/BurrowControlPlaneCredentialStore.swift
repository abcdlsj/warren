...
enum BurrowControlPlaneCredentialStore {
    private static let service = "com.abcdlsj.burrow.control-plane"

    static func loadOrImport(
        environment: [String: String]
    ) -> (url: URL, hostID: String, credential: String)? {
        if let rawURL = environment["BURROW_CONTROL_PLANE_URL"],
           let url = validatedRelayURL(rawURL),
           let hostID = environment["BURROW_CONTROL_PLANE_HOST_ID"],
           UUID(uuidString: hostID) != nil,
           let credential = environment["BURROW_CONTROL_PLANE_HOST_TOKEN"],
           !credential.isEmpty {
